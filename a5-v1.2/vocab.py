...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        tensor = torch.tensor(sents_padded)
        sents_var = tensor.permute(1,0,2)

        return sents_var.contiguous()

        #sents_var = torch.tensor(sents_padded, device = device)
        #tensorFromList = torch.FloatTensor(listTryingToConvert)

        ### YOUR CODE HERE for part 1g
        ###     Connect `words2charindices()` and `pad_sents_char()` which you've defined in
        ###     previous parts

        # output shape of pad_sents_char = Output shape: (batch_size, max_sentence_length, max_word_length)
